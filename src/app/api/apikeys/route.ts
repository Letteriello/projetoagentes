...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!serviceName || typeof serviceName !== 'string' || serviceName.trim() === "") {
      return NextResponse.json({ error: 'Service name is required and must be a non-empty string.' }, { status: 400 });
    }

    console.log(`API Route: Received request to register service: ${serviceName}`);
    ...
    console.log(`API Route: Service "${serviceName}" registered successfully with ID ${newService.id}.`);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: "DELETE method for specific ID should be in [id]/route.ts" }, { status: 405 });
  } catch (error) {
