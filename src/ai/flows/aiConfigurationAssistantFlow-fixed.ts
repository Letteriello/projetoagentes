...
export const aiConfigurationAssistantFlow = ai.defineFlow(
  ...
  async (input: z.infer<typeof AiConfigurationAssistantInputSchema>, request: ActionContext) => {
...
function parseLlmJsonResponse(responseText: string, expectedKeys: string[]): any {
  try {
...
async function getWorkflowDetailedType(query: string): Promise<WorkflowDetailedType> {
  // Implementation...
  return "sequential"; // Default value
}
