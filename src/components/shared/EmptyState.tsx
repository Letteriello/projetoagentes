...
export interface EmptyStateProps {
  ...
  illustration?: string | React.ReactNode; // Added illustration prop
  actionButton?: {
    text: string;
    onClick: () => void;
    icon?: React.ReactNode;
...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  description,
  icon,
  illustration, // Added illustration to destructuring
  actionButton,
  className,
}) => {
  return (
      ...
      {illustration && (
        <div className="mb-4">
          {typeof illustration === 'string' ? (
            <img src={illustration} alt={title} className="h-32 w-32 mx-auto text-muted-foreground" />
          ) : (
            React.isValidElement(illustration) ? React.cloneElement(illustration as React.ReactElement<any>, { className: cn("h-32 w-32 mx-auto text-muted-foreground", (illustration.props as any)?.className) }) : illustration
          )}
        </div>
      )}
