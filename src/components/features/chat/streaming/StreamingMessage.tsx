...
export function StreamingMessage({
  ...
  const renderIcon = () => {
    switch (role) {
      ...
      case 'tool':
        return <div className="h-6 w-6 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold">T</div>;
      default:
        return <Bot className="h-6 w-6 text-primary" />;
    }
  };
          ...
          components={{
            code({node, inline, className, children, ...props}) {
              ...
              return !inline && match ? (
                ...
                <code className="block bg-muted p-4 rounded-md text-sm my-3 whitespace-pre-wrap" {...props}>
                  {children}
                </code>
              );
            },
            ul({node, ...props}) {
              return <ul className="list-disc pl-6 my-3 space-y-1" {...props} />;
            },
            ol({node, ...props}) {
              return <ol className="list-decimal pl-6 my-3 space-y-1" {...props} />;
            },
            li({node, ...props}) {
