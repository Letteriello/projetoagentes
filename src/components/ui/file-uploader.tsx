...
import { UploadCloud, File as FileIcon, X as XIcon, Image as ImageIcon } from "lucide-react";
...
export const FileUploader: React.FC<FileUploaderProps> = ({
    ...
    if (rejected && rejected.length > 0) {
      ...
      rejected.forEach((rejection: any) => {
        rejection.errors.forEach((error: any) => {
          ...
          toast({ variant: "destructive", title: "Erro no Arquivo", description: message });
        });
      });
  ...
  const baseStyle = "border-2 border-dashed rounded-md p-6 text-center cursor-pointer transition-colors duration-200 ease-in-out";
    ...
    <div className="w-full">
      <div {...getRootProps()} className={dzClassName}>
        ...
        {children ? (
          ...
          <div className="flex flex-col items-center justify-center space-y-2">
            ...
            {maxFileSize !== Infinity && (
              <p className="text-xs text-muted-foreground/80">Tamanho máximo: {maxFileSize / (1024*1024)}MB</p>
            )}
            {maxFiles > 1 && (
              <p className="text-xs text-muted-foreground/80">Máximo de arquivos: {maxFiles}</p>
            )}
          </div>
        )}
      </div>
      ...
      {fileErrors.length > 0 && (
        <div className="mt-2 space-y-1">
          ...
          {fileErrors.length > 3 && <p className="text-xs text-destructive">...e mais {fileErrors.length - 3} erros.</p>}
        </div>
      )}
          ...
          <ul className="divide-y divide-border rounded-md border">
            {selectedFiles.map((file, index) => (
              <li key={index} className="flex items-center justify-between p-2 text-sm">
                ...
                {!disabled && (
                  <Button variant="ghost" size="icon" onClick={() => removeFile(file)} aria-label={`Remover ${file.name}`}>
