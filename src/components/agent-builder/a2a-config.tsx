...
export function A2AConfig({
  ...
  const handleUpdateChannel = (index: number, updated: CommunicationChannel) => {
    ...
    <div className="space-y-6">
          ...
          <CardContent className="p-4 space-y-4">
            {a2aConfig.communicationChannels.length === 0 ? (
              <div className="text-center py-4 text-sm text-muted-foreground">
                Nenhum canal definido. Adicione canais para permitir comunicação com outros agentes.
              </div>
            ) : (
              a2aConfig.communicationChannels.map((channel, index) => (
                <div key={index} className="border rounded-md p-3 space-y-3">
                  <div className="flex justify-between items-center">
                    <Input 
                      value={channel.name}
                      onChange={(e) => handleUpdateChannel(index, {...channel, name: e.target.value})}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => handleUpdateChannel(index, {
                          ...channel, 
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label htmlFor="defaultResponseFormat" className="text-xs">Formato de Resposta Padrão</Label>
            <Select 
              value={a2aConfig.defaultResponseFormat} 
              onValueChange={(value) => setA2AConfig({...a2aConfig, defaultResponseFormat: value as 'json' | 'text'})}
            >
          ...
          <div>
            <Label htmlFor="maxMessageSize" className="text-xs">Tamanho Máximo de Mensagem (bytes)</Label>
            <Input 
              id="maxMessageSize" 
              type="number" 
              value={a2aConfig.maxMessageSize || ''} 
              onChange={(e) => setA2AConfig({...a2aConfig, maxMessageSize: e.target.value ? parseInt(e.target.value) : undefined})}
              className="h-8"
            />
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) => setA2AConfig({...a2aConfig, loggingEnabled: checked})}
          />
