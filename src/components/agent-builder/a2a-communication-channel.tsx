...
export const CommunicationChannelItem: React.FC<CommunicationChannelItemProps> = ({ 
    channel, 
    onUpdate, 
    onDelete,
    availableAgents = []
    ...
    return (
        <div className="border rounded-md p-3 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div>
                    <Label htmlFor={`channel-direction-${channel.id}`} className="text-xs">Direção</Label>
                    <Select 
                        value={channel.direction} 
                        onValueChange={(value) => onUpdate({...channel, direction: value as 'inbound' | 'outbound' | 'bidirectional'})}
                    >
                        <SelectTrigger id={`channel-direction-${channel.id}`} className="h-8">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div>
                    <Label htmlFor={`channel-format-${channel.id}`} className="text-xs">Formato</Label>
                    <Select 
                        value={channel.messageFormat} 
                        onValueChange={(value) => onUpdate({...channel, messageFormat: value as 'json' | 'text' | 'binary'})}
                    >
            ...
            <div className="grid grid-cols-2 gap-3">
                <div>
                    <Label htmlFor={`channel-sync-${channel.id}`} className="text-xs">Modo de Sincronização</Label>
                    <Select 
                        value={channel.syncMode} 
                        onValueChange={(value) => onUpdate({...channel, syncMode: value as 'sync' | 'async'})}
                    >
                ...
                {channel.syncMode === 'sync' && (
                    <div>
                        <Label htmlFor={`channel-timeout-${channel.id}`} className="text-xs">Timeout (ms)</Label>
                        <Input 
                            id={`channel-timeout-${channel.id}`}
                            type="number"
                            value={channel.timeout || ''}
                            onChange={(e) => onUpdate({...channel, timeout: e.target.value ? parseInt(e.target.value) : undefined})}
                            className="h-8"
                            placeholder="ex: 5000"
                        />
                    </div>
            ...
            {(channel.direction === 'outbound' || channel.direction === 'bidirectional') && (
                <div>
                    <Label htmlFor={`channel-target-${channel.id}`} className="text-xs">Agente Alvo</Label>
                    <Select 
                        value={channel.targetAgentId || ''} 
                        onValueChange={(value) => onUpdate({...channel, targetAgentId: value || undefined})}
                    >
            ...
            {channel.messageFormat === 'json' && (
                <div>
                    <Label htmlFor={`channel-schema-${channel.id}`} className="text-xs">Schema JSON (opcional)</Label>
                    <Input 
                        id={`channel-schema-${channel.id}`}
                        value={channel.schema || ''}
                        onChange={(e) => onUpdate({...channel, schema: e.target.value || undefined})}
                        className="h-8"
